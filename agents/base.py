...
from core.models import AnalysisResult, EvidencePacket
...
class BaseAgent(ABC):
    ...
    async def process(self, packet: EvidencePacket) -> EvidencePacket:
        ...
        try:
            ...
            logger.debug(f"{self.agent_id} processed packet {packet.id} in {processing_time:.2f}ms")
            ...
            error_result = AnalysisResult(
                agent_id=self.agent_id, agent_type=self.agent_type, confidence=0.0, errors=[str(e)]
            )
    ...
    def get_metrics(self) -> dict[str, Any]:
        ...
        if metrics["packets_processed"] > 0:
            metrics["avg_processing_time_ms"] = (
                metrics["total_processing_time_ms"] / metrics["packets_processed"]
            )
        return metrics
...
def make_remote(agent: BaseAgent, **ray_options) -> Any:
    ...
    return RemoteAgent.remote(
        agent_id=agent.agent_id, agent_type=agent.agent_type, config=agent.config
    )
